...
SQLite repositories держат один `sqlite3.Connection` на repository instance. Flask dev server может обрабатывать запросы в несколько потоков, поэтому операции `execute`/`commit` защищены `RLock`: если один connection разрешен к использованию из разных потоков (`check_same_thread=False`), доступ к нему должен быть сериализован. `RLock` выбран вместо обычного `Lock`, потому что некоторые операции могут вызывать другие методы того же repository, например удаление истекшей session при чтении.

Static-запросы к `/assets/*` не пересохраняют неизмененную session. Это убирает лишние записи в SQLite при параллельной загрузке CSS/JS и снижает вероятность конкуренции на session store, но не заменяет блокировку: параллельные не-static запросы все равно могут одновременно работать с тем же connection.
