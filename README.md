...
SQLite repositories используют общий пул `sqlite3.Connection` на приложение. Каждый запрос к хранилищу берет соединение из пула на время операции, поэтому repositories не держат общий connection под ручной блокировкой. Checkout из пула ограничен timeout, а сами соединения настроены с `journal_mode=WAL` и `busy_timeout`.
